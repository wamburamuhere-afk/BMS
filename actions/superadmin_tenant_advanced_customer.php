<?php
/**
 * actions/superadmin_tenant_advanced_customer.php — view/set ONE tenant's
 * "Advanced Customer" override from the superadmin Tenant Detail page.
 *
 * Thin wrapper around core/tenant_admin.php::tenantAdvancedCustomerStatus()/
 * setTenantAdvancedCustomer() — direct copy of
 * actions/superadmin_tenant_advanced_product.php's own structure, its own
...
if ($action === 'status') {
    $status = tenantAdvancedCustomerStatus($tenantId);
    if ($status === null) {
        http_response_code(422);
        echo json_encode(['success' => false, 'message' => 'Could not read Advanced Customer status for this tenant right now.']);
        exit;
    }
...
if ($action === 'set') {
    ...
    $r = setTenantAdvancedCustomer($tenantId, $enabled, $locked);
    if (!$r['ok']) {
        http_response_code(422);
        echo json_encode(['success' => false, 'message' => $r['error']]);
        exit;
    }
    echo json_encode(['success' => true, 'message' => 'Advanced Customer updated for this tenant.']);
    exit;
}
