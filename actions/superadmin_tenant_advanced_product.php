<?php
/**
 * actions/superadmin_tenant_advanced_product.php — view/set ONE tenant's
 * "Advanced Product" override from the superadmin Tenant Detail page.
 *
 * Thin wrapper around core/tenant_admin.php::tenantAdvancedProductStatus()/
 * setTenantAdvancedProduct() — same narrow "opens a tenant's own database"
 * exception as actions/superadmin_tenant_pos_simple_mode.php, its own
...
if ($action === 'status') {
    $status = tenantAdvancedProductStatus($tenantId);
    if ($status === null) {
        http_response_code(422);
        echo json_encode(['success' => false, 'message' => 'Could not read Advanced Product status for this tenant right now.']);
        exit;
    }
...
if ($action === 'set') {
    ...
    $r = setTenantAdvancedProduct($tenantId, $enabled, $locked);
    if (!$r['ok']) {
        http_response_code(422);
        echo json_encode(['success' => false, 'message' => $r['error']]);
        exit;
    }
    echo json_encode(['success' => true, 'message' => 'Advanced Product updated for this tenant.']);
    exit;
}
