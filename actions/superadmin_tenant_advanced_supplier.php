<?php
/**
 * actions/superadmin_tenant_advanced_supplier.php — view/set ONE tenant's
 * "Advanced Supplier" override from the superadmin Tenant Detail page.
 *
 * Thin wrapper around core/tenant_admin.php::tenantAdvancedSupplierStatus()/
 * setTenantAdvancedSupplier() — direct copy of
...
if ($action === 'status') {
    $status = tenantAdvancedSupplierStatus($tenantId);
    if ($status === null) {
        http_response_code(422);
        echo json_encode(['success' => false, 'message' => 'Could not read Advanced Supplier status for this tenant right now.']);
        exit;
    }
...
if ($action === 'set') {
    ...
    $r = setTenantAdvancedSupplier($tenantId, $enabled, $locked);
    if (!$r['ok']) {
        http_response_code(422);
        echo json_encode(['success' => false, 'message' => $r['error']]);
        exit;
    }
    echo json_encode(['success' => true, 'message' => 'Advanced Supplier updated for this tenant.']);
    exit;
}
