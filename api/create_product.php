...
try {
    ...
    $product_data = [
        ...
        // Phase 25 (pos_upgrade_plan.md §9) — see api/update_product.php for context.
    ...
    if ($product_data['sku']) {
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM products WHERE sku = ? OR product_code = ?");
        $stmt->execute([$product_data['sku'], $product_data['sku']]);
        if ($stmt->fetchColumn() > 0) {
    ...
    logActivity($pdo, $user_id, 'Create product', "User created a new product: {$product_data['product_name']}" . ($product_data['sku'] ? " ({$product_data['sku']})" : "") . " (ID $product_id)");
