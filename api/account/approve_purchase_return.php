...
// api/update_purchase_return_status.php. We re-use that file's adjustStock()
// helper by require-ing it (function_exists guard prevents redefinition).
// After deduction we set stock_updated=1 so the existing reversal logic in
// the legacy status endpoint continues to work if the return is later
// rejected/cancelled.
...
try {
    ...
    // Inlined here (and at top of file) to avoid require-ing
    // api/update_purchase_return_status.php which has top-level POST handling
    // we'd accidentally re-execute. The function body is byte-identical to the
    // helper in that legacy file. function_exists guard prevents redefinition.
...
/**
 * Deduct or add stock for every line item in a purchase return.
 * Behaviour is byte-identical to adjustStock() in
 * api/update_purchase_return_status.php — kept under a different name here
 * to avoid any collision if both files end up loaded in the same request.
 */
