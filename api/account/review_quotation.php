...
try {
    ...
    // Phase C — block reviews against quotations on projects not in user scope
    ...
    workflowCaptureSignature($pdo, 'quotation', $id, 'reviewed',
        $_SESSION['user_id'], $actor['name'], $actor['role']);

    logActivity($pdo, $_SESSION['user_id'], 'Review Quotation',
        "{$actor['name']} marked Quotation #{$quote['order_number']} as reviewed");
