...
// Delivery Order's own Convoy-layout accent color. Configurable from
// System Settings > Color Setting; falls back to the original design color when unset.
$accent = getSetting('print_template_color_do_convoy', '#374151');
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* ── SECTION LABEL BARS ── */
        .section-label { background: var(--accent); color: #fff; font-size: 10.5px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.6px; padding: 6px 12px; print-color-adjust: exact; -webkit-print-color-adjust: exact; }
        ...
        /* ── TRANSPORT STRIP (convoy = driver/vehicle front and centre) ── */
        .transport-strip { display: flex; gap: 2px; margin-bottom: 18px; }
        .transport-strip > div { width: 33.33%; border: 1px solid var(--accent); padding: 8px 12px; }
        .transport-strip .t-label { font-size: 9.5px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: var(--accent); margin-bottom: 3px; }
        .transport-strip .t-value { font-size: 12.5px; font-weight: 700; }
...
<body onload="bmsAutoFitPrint()">
    ...
    <div class="company-strip">
        ...
        <div>
            ...
            <?php
            $strip = [];
            if (!empty($comp['address'])) $strip[] = htmlspecialchars($comp['address']);
            if (!empty($comp['phone']))   $strip[] = 'Tel: ' . htmlspecialchars($comp['phone']);
            if (!empty($comp['email']))   $strip[] = htmlspecialchars($comp['email']);
            if ($strip): ?><p><?= implode(' &nbsp;|&nbsp; ', $strip) ?></p><?php endif; ?>
        </div>
    </div>
    ...
    <div class="transport-strip">
        <div>
            ...
            <div class="status-badge"><?= htmlspecialchars(str_replace('_', ' ', strtoupper($do['status']))) ?></div>
        </div>
        <div>
            <div class="t-label">Driver</div>
            <div class="t-value"><?= !empty($do['driver_name']) ? htmlspecialchars($do['driver_name']) : '—' ?></div>
        </div>
        <div>
            <div class="t-label">Vehicle Reg</div>
            <div class="t-value"><?= !empty($do['vehicle_number']) ? htmlspecialchars($do['vehicle_number']) : '—' ?></div>
        </div>
    </div>
    <?php if (!empty($do['contact_person']) || !empty($do['expected_date'])): ?>
    ...
    <div class="section-label">Delivery Notes Issued</div>
    <table style="margin-bottom:20px;">
        <thead>
            <tr>
                <th style="width:38%;">DN Number</th>
