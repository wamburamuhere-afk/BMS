...
class TCPDFBarcode {
	...
	public function getBarcodePngData($w=2, $h=30, $color=array(0,0,0)) {
		...
		} elseif (extension_loaded('imagick') && class_exists('Imagick') && class_exists('ImagickPixel') && class_exists('ImagickDraw')) {
			$imagick = true;
		...
		if ($imagick && ($png instanceof Imagick) && ($bar instanceof ImagickDraw)) {
			$png->drawImage($bar);
			return $png;
		} else {
			ob_start();
