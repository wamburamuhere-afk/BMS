...
class TCPDF_STATIC {
	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function objclone($object) {
		if (class_exists('Imagick') AND ($object instanceof Imagick) AND (version_compare(phpversion('imagick'), '3.0.1') !== 1)) {
	...
	public static function _AES($key, $text) {
		...
		if (function_exists('mcrypt_encrypt') AND defined('MCRYPT_RIJNDAEL_128') AND defined('MCRYPT_MODE_CBC') AND defined('MCRYPT_RAND')) {
			$iv = call_user_func('mcrypt_create_iv', call_user_func('mcrypt_get_iv_size', constant('MCRYPT_RIJNDAEL_128'), constant('MCRYPT_MODE_CBC')), constant('MCRYPT_RAND'));
			$text = call_user_func('mcrypt_encrypt', constant('MCRYPT_RIJNDAEL_128'), $key, $text, constant('MCRYPT_MODE_CBC'), $iv);
			$text = $iv.$text;
			return $text;
		}
		return '';
	...
	public static function _AESnopad($key, $text) {
		...
		if (function_exists('mcrypt_encrypt') AND defined('MCRYPT_RIJNDAEL_128') AND defined('MCRYPT_MODE_CBC')) {
			$iv = str_repeat("\x00", call_user_func('mcrypt_get_iv_size', constant('MCRYPT_RIJNDAEL_128'), constant('MCRYPT_MODE_CBC')));
			$text = call_user_func('mcrypt_encrypt', constant('MCRYPT_RIJNDAEL_128'), $key, $text, constant('MCRYPT_MODE_CBC'), $iv);
			return $text;
		}
		return '';
	...
	public static function _RC4($key, $text, &$last_enc_key, &$last_enc_key_c) {
		if (function_exists('mcrypt_encrypt') AND defined('MCRYPT_ARCFOUR') AND defined('MCRYPT_MODE_STREAM') AND ($out = @call_user_func('mcrypt_encrypt', constant('MCRYPT_ARCFOUR'), $key, $text, constant('MCRYPT_MODE_STREAM'), ''))) {
