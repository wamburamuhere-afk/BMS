...
class TCPDF {
	...
	public function AddFont($family, $style='', $fontfile='', $subset='default') {
		...
		if (!TCPDF_STATIC::empty_string($file)) {
			...
			if (!isset($this->FontFiles[$file]['fontkeys']) || !is_array($this->FontFiles[$file]['fontkeys'])) {
				$this->FontFiles[$file]['fontkeys'] = array();
			}
	...
	public function setFont($family, $style='', $size=null, $fontfile='', $subset='default', $out=true) {
		...
		if (isset($this->CurrentFont['fontkey']) AND isset($this->CurrentFont['subsetchars'])) {
			...
			$subsetchars = is_array($this->CurrentFont['subsetchars']) ? $this->CurrentFont['subsetchars'] : array_fill(0, 255, true);
			$this->setFontSubBuffer($this->CurrentFont['fontkey'], 'subsetchars', $subsetchars);
		}
	...
	public function Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M') {
		...
		if ($this->txtshadow['enabled']) {
			...
			if (is_array($this->txtshadow['color'])) {
			...
			$this->x = $x;
			$this->y = $y;
			if (is_array($bc)) { $this->setFillColorArray($bc); }
			if (is_array($fc)) { $this->setTextColorArray($fc); }
			if (is_array($sc)) { $this->setDrawColorArray($sc); }
			if ($this->txtshadow['opacity'] != $alpha['CA']) {
		...
		if ($txt != '') {
			...
			if ($this->isunicode) {
				if (($this->CurrentFont['type'] == 'core') OR ($this->CurrentFont['type'] == 'TrueType') OR ($this->CurrentFont['type'] == 'Type1')) {
					...
					if (isset($this->CurrentFont['fontkey'])) {
						$fontkey = $this->CurrentFont['fontkey'];
					}
					$this->setFontSubBuffer($fontkey, 'subsetchars', $this->CurrentFont['subsetchars']);
			...
			if (isset($uniblock) AND is_array($uniblock)) {
			...
			if (!TCPDF_STATIC::empty_string($fontfile)) {
				...
				if ((!$compressed) AND (isset($info['length2']))) {
					...
					foreach ($info['fontkeys'] as $fontkey) {
						...
						if (is_array($fontinfo['subsetchars'])) {
							foreach($fontinfo['subsetchars'] as $sck => $scv) {
								$subsetchars[$sck] = $scv;
							}
						}
					}
	...
	protected function _putimages() {
		...
		foreach ($this->imagekeys as $file) {
			...
			$altoid = null;
			if ((!$this->pdfa_mode) AND isset($info['altimgs']) AND is_array($info['altimgs'])) {
				$altoid = $this->_newobj();
				$out = '[';
			...
			$out .= ' /Width '.$info['w'];
			$out .= ' /Height '.$info['h'];
			if (is_array($info) AND array_key_exists('masked', $info)) {
	...
	protected function _datestring($n=0, $timestamp=0) {
		...
		return (string) $this->_datastring('D:'.TCPDF_STATIC::getFormattedDate($timestamp), $n);
	}
	...
	protected function _generateencryptionkey() {
		...
		if (!$this->encryptdata['pubkey']) { // standard mode
			...
			foreach ($this->encryptdata['pubkeys'] as $pubkey) {
				...
				if (isset($pubkey['p'])) {
					$pkp = is_array($pubkey['p']) ? $pubkey['p'] : array();
					$pkprotection = TCPDF_STATIC::getUserPermissionCode($pkp, $this->encryptdata['mode']);
				} else {
	...
	public function setProtection($permissions=array('print', 'modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble', 'print-high'), $user_pass='', $owner_pass=null, $mode=0, $pubkeys=null) {
		...
		if ($mode > 1) { // AES
			...
			if (extension_loaded('mcrypt') && function_exists('mcrypt_get_cipher_name') && defined('MCRYPT_RIJNDAEL_128') && call_user_func('mcrypt_get_cipher_name', constant('MCRYPT_RIJNDAEL_128')) === false) {
	...
	public function write1DBarcode($code, $type, $x=null, $y=null, $w=null, $h=null, $xres=null, $style=array(), $align='') {
		...
		if ($style['bgcolor']) {
			$bgc = is_array($style['bgcolor']) ? $style['bgcolor'] : array();
			$this->Rect($xpos_rect, $y, $w, $h, $style['border'] ? 'DF' : 'F', array(), $bgc);
		} elseif ($style['border']) {
	...
	public function write2DBarcode($code, $type, $x=null, $y=null, $w=null, $h=null, $style=array(), $align='', $distort=false) {
		...
		if ($style['bgcolor']) {
			$bgc = is_array($style['bgcolor']) ? $style['bgcolor'] : array();
			$this->Rect($xpos, $y, $w, $h, $style['border'] ? 'DF' : 'F', array(), $bgc);
		} elseif ($style['border']) {
	...
	public function addTOC($page=null, $numbersfont='', $filler='.', $toc_name='TOC', $style='', $color=array(0,0,0)) {
		...
		foreach ($this->outlines as $key => $outline) {
			...
			$this->Write(0, $txt, $link, false, $aligntext, false, 0, false, false, 0, $numwidth, array());
			if ($this->rtl) {
	...
	*/
	...
	public function getFontFamilyName($fontfamily) {
		...
		if (isset($this->CurrentFont['fontkey'])) {
			return (string) $this->CurrentFont['fontkey'];
		}
	...
	public function printTemplate($id, $x=null, $y=null, $w=0, $h=0, $align='', $palign='', $fitonpage=false) {
		...
		if (!empty($this->xobjects[$id]['annotations'])) {
			foreach ($this->xobjects[$id]['annotations'] as $annot) {
				...
				$annot_opt = is_array($annot['opt']) ? $annot['opt'] : array('Subtype'=>'Text');
				$this->Annotation($ax, $ay, $aw, $ah, $annot['text'], $annot_opt, $annot['spaces']);
			}
		}
		// set pointer to align the next text/objects
		switch($align) {
	...
	protected function setSVGStyles($svgstyle, $prevsvgstyle, $x=0, $y=0, $w=1, $h=1, $clip_function='', $clip_params=array()) {
		...
		if (preg_match('/url\([\s]*\#([^\)]*)\)/si', $svgstyle['fill'], $regs)) {
			...
			if (((is_array($gradient['stops']) || $gradient['stops'] instanceof Countable) && count($gradient['stops']) > 1) AND is_array($gradient['coords'])) {
