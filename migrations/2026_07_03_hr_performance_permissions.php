...
echo "Starting migration: HR performance & training permissions (Tier 3)...\n";

try {
    // Full access = admin-flagged roles + whichever roles hold can_edit on
    // 'employees' on THIS server — resolved at runtime, never hard-coded ids
    // (live-system rule). Everyone else = view-only.
    $fullRoles = $pdo->query("
    ...
    $hasSubmit  = (bool)$pdo->query("SHOW COLUMNS FROM role_permissions LIKE 'can_submit'")->fetch();
    ...
    foreach ($pages as $key => $name) {
        ...
        foreach ($roles as $rid) {
            ...
            $marks = '?, ?, ?, ?, ?, ?';
            if ($hasSubmit)  { $cols .= ', can_submit';  $marks .= ', ?'; $vals[] = $isFull ? 1 : 0; }
