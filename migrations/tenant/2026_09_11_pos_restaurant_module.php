 ...
 *
 * Phase 30 (pos_upgrade_plan.md §9) — Restaurant Module + POS Navigation
 * Reorganization. This migration ships the BACKEND/SCHEMA half only (the
 * UI/nav/tests half follows in a later commit).
 *
 * warehouses.pos_mode is the one switch this whole tier is organized around
 * — DEFAULT 'retail' preserves current behaviour for every warehouse,
 * everywhere, permanently, unless an admin explicitly changes it.
 *
 * Recipes need ZERO new schema/code: a recipe-based dish is simply a normal
 * `is_combo=1` product (Phase 23) with `kitchen_station_id` set — ingredient
 * consumption already works via core/pos_combo_products.php.
 *
 * Every existing warehouse gets a default floor ("Main Floor") + a default
 * kitchen station ("Main Kitchen") seeded here — inert until pos_mode is
 * changed away from 'retail', but present so the restaurant UI (built in the
 * next commit) never renders against an empty floor/station list.
 */
...
echo "Starting tenant migration: POS restaurant module (Phase 30 backend)...\n";
...
try {
    ...
    // ── products.kitchen_station_id ──────────────────────────────────────
    $col = $pdo->query("SHOW COLUMNS FROM products LIKE 'kitchen_station_id'")->fetch();
    if (!$col) {
        $pdo->exec("ALTER TABLE products ADD COLUMN kitchen_station_id INT NULL DEFAULT NULL AFTER track_serials");
        echo "  + products.kitchen_station_id added.\n";
    } else {
    ...
    // ── pos_sale_item_modifiers — identical child-table shape to
    //    pos_sale_item_batches / pos_sale_item_serials ──────────────────
    $pdo->exec("
    ...
    // ── pos_sales: reuse dormant sale_type ENUM + new table_id/assigned_to ─
    ...
    // ── pos_held_sales: warehouse_id + table_id (table-addressable holds) ─
    $col = $pdo->query("SHOW COLUMNS FROM pos_held_sales LIKE 'warehouse_id'")->fetch();
    if (!$col) {
        $pdo->exec("ALTER TABLE pos_held_sales ADD COLUMN warehouse_id INT NULL DEFAULT NULL AFTER customer_id");
    ...
    if (!$col) {
        $pdo->exec("ALTER TABLE pos_held_sales ADD COLUMN table_id INT NULL DEFAULT NULL AFTER warehouse_id");
        echo "  + pos_held_sales.table_id added.\n";
    } else {
        echo "  · pos_held_sales.table_id already present.\n";
    }

    // ── restaurant_reservations / restaurant_reservation_reminders ──────
    $pdo->exec("
    ...
    // ── notification_events row for reservation reminders ────────────────
    ...
    // ── restaurant_pos permission row (feature-registry gate) ────────────
    ...
    // ── Seed a default floor + kitchen station for every existing
    //    warehouse. Inert until pos_mode is changed away from 'retail', but
    //    present so the restaurant UI never renders against an empty list.
