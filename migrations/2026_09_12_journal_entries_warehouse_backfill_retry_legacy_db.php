 ...
 *
 * Mirrors migrations/tenant/2026_09_12_journal_entries_warehouse_backfill_retry.php
 * onto the LEGACY / non-tenant database. See that file's doc comment for the
 * full root-cause explanation: 2026-09-11's backfill migration ran BEFORE
 * that day's column-add migration (alphabetical filename collision on the
 * same date prefix — "backfill" < "id"), so every one of its UPDATEs failed
 * with "Unknown column 'je.warehouse_id'" and was silently skipped. Confirmed
 * in this server's own deploy log. This migration is not filename-order
 * dependent — it verifies the column itself before backfilling — and treats
 * a real failure as fatal instead of swallowing it.
 */
...
try {
    ...
    if (!$hasCol) {
        echo "  ! warehouse_id column missing — adding it now.\n";
