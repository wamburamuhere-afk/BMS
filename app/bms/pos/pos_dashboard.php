<?php
// File: pos_dashboard.php — POS Workspace (Dashboard + Sales History in one, toggled)
...
// Sales-history default window: last 12 months → today (so existing data is never
// hidden by a current-month-only default).
$hist_from = date('Y-m-d', strtotime('-12 months'));
$hist_to   = date('Y-m-d');
?>
<div class="container-fluid mt-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
        ...
        <div class="d-flex gap-2 flex-wrap">
            <div class="btn-group" role="group" aria-label="POS view toggle">
                <button type="button" class="btn btn-primary" id="btnViewDashboard"><i class="bi bi-speedometer2 me-1"></i> Dashboard</button>
                <button type="button" class="btn btn-outline-primary" id="btnViewHistory"><i class="bi bi-receipt me-1"></i> Sales History</button>
            </div>
    ...
    <!-- ========================= SALES HISTORY PANE ========================= -->
    <div id="paneHistory" class="d-none">
        <!-- Date filter -->
            ...
            <div class="card-body py-2">
                <div class="row g-2 align-items-end">
                    <div class="col-6 col-md-3">
                        ...
                        <input type="date" id="fFrom" class="form-control form-control-sm" value="<?= $hist_from ?>">
                    </div>
                    <div class="col-6 col-md-3">
                        <label class="form-label small mb-1">To</label>
                        <input type="date" id="fTo" class="form-control form-control-sm" value="<?= $hist_to ?>">
                    </div>
        ...
        <!-- Desktop table — white header, blue column titles, S/NO first -->
        <div id="tableView">
                ...
                <div class="modal-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-sm align-middle">
                            <thead>
                                <tr class="text-primary"><th class="text-primary">Product</th><th class="text-primary text-end">Sold</th><th class="text-primary text-end">Returnable</th><th class="text-primary" style="width:120px">Return Qty</th></tr>
                            </thead>
...
function showDashboard() {
    ...
    $('#btnViewDashboard').removeClass('btn-outline-primary').addClass('btn-primary');
    $('#btnViewHistory').removeClass('btn-primary').addClass('btn-outline-primary');
}
function showHistory() {
    $('#paneHistory').removeClass('d-none');
    $('#paneDashboard').addClass('d-none');
    $('#btnViewHistory').removeClass('btn-outline-primary').addClass('btn-primary');
    $('#btnViewDashboard').removeClass('btn-primary').addClass('btn-outline-primary');
    if (!historyLoaded) { initHistoryTable(); loadSales(); historyLoaded = true; }
    if (table) { table.columns.adjust(); applyView(); }
}

$(document).ready(function () {
    loadDashboard();
    $('#btnViewDashboard').on('click', showDashboard);
    $('#btnViewHistory').on('click', showHistory);
    bindHistoryHandlers();
});
...
function loadDashboard() {
    $.getJSON(DASH_URL, function (res) {
        ...
        trendChart = new Chart(ctx, {
            type: 'bar',
            data: { labels: d.trend.map(t => t.label), datasets: [{ label: 'Net Sales (' + CURRENCY + ')', data: d.trend.map(t => t.net), backgroundColor: '#0d6efd', borderRadius: 4 }] },
            options: {
...
function initHistoryTable() {
    table = $('#posSalesTable').DataTable({
        ...
        columns: [
            { data: null, orderable: false, className: 'text-muted', render: (d, t, r, meta) => meta.row + 1 },
...
function bindHistoryHandlers() {
    ...
    $(window).on('resize', function () { if (!$('#paneHistory').hasClass('d-none')) applyView(); });
    ...
    $('#returnForm').on('submit', function (e) {
        ...
        $('#ret_lines tr').each(function () {
            const iid = $(this).data('iid'); const qty = parseFloat($(this).find('.ret-qty').val()) || 0;
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.success) { bootstrap.Modal.getInstance(document.getElementById('returnModal')).hide(); loadSales(); loadDashboard(); Swal.fire({ icon: 'success', title: 'Return processed', text: res.message, timer: 2200, showConfirmButton: false }); }
    ...
    $('#receiveForm').on('submit', function (e) {
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.success) { bootstrap.Modal.getInstance(document.getElementById('receiveModal')).hide(); loadSales(); Swal.fire({ icon: 'success', title: 'Payment received', text: res.message, timer: 2200, showConfirmButton: false }); }
                else { Swal.fire({ icon: 'error', title: 'Error', text: res.message || 'Failed.' }); }
            },
            error: function () { Swal.fire({ icon: 'error', title: 'Error', text: 'Server error.' }); },
            complete: function () { btn.prop('disabled', false).html(orig); }
