...
<div class="container-fluid mt-4">
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-body text-center py-5">
            ...
            <h5><?= t('Your account isn\'t linked to an employee record yet') ?></h5>
...
function loadProfile(){ $.getJSON(MH_API, { section:'profile' }, function(r){ if (!r.success){ $('#pane-profile .card-body').html('Could not load.'); return; } const d=r.data;
    $('#pane-profile').html(`<div class="card-body"><div class="row">
        <div class="col-md-6"><table class="table table-sm">
            <tr><th style="width:40%"><?= t('Name') ?></th><td>${esc((d.first_name||'')+' '+(d.middle_name||'')+' '+(d.last_name||''))}</td></tr>
            <tr><th><?= t('Employee #') ?></th><td>${esc(d.employee_number)}</td></tr>
            <tr><th><?= t('Designation') ?></th><td>${esc(d.designation_name||'—')}</td></tr>
            <tr><th><?= t('Department') ?></th><td>${esc(d.department_name||'—')}</td></tr>
            <tr><th><?= t('Project') ?></th><td>${esc(d.project_name||'—')}</td></tr></table></div>
        <div class="col-md-6"><table class="table table-sm">
            <tr><th style="width:40%"><?= t('Email') ?></th><td>${esc(d.email)}</td></tr>
            <tr><th><?= t('Phone') ?></th><td>${esc(d.phone)}</td></tr>
            <tr><th><?= t('Hire date') ?></th><td>${esc(d.hire_date||'—')}</td></tr>
            <tr><th><?= t('Status') ?></th><td>${esc(d.employment_status)}</td></tr></table></div></div>
...
function loadPayslips(){ $.getJSON(MH_API, { section:'payslips' }, function(r){ if (!r.success) return;
    ...
    $('#pane-payslips').html(`<div class="card-body"><table class="table table-sm"><thead><tr><th><?= t('Period') ?></th><th><?= t('Date') ?></th><th><?= t('Net') ?></th><th><?= t('Status') ?></th></tr></thead><tbody>${rows}</tbody></table></div>`);
}); }
function loadLeave(){ $.getJSON(MH_API, { section:'leave' }, function(r){ if (!r.success) return;
    ...
    $('#pane-leave').html(`<div class="card border-0 shadow-sm"><div class="card-body">
        ...
        <h6><?= t('Balances (this year)') ?></h6><table class="table table-sm"><thead><tr><th><?= t('Type') ?></th><th><?= t('Entitled') ?></th><th><?= t('Carried over') ?></th></tr></thead><tbody>${bal}</tbody></table>
        <h6 class="mt-3"><?= t('History') ?></h6><table class="table table-sm"><thead><tr><th><?= t('Type') ?></th><th><?= t('Dates') ?></th><th><?= t('Days') ?></th><th><?= t('Status') ?></th></tr></thead><tbody>${hist}</tbody></table></div></div>`);
}); }
function loadDocs(){ $.getJSON(MH_API, { section:'documents' }, function(r){ if (!r.success) return;
    ...
    $('#pane-docs').html(`<div class="card-body"><h6><?= t('My Documents') ?></h6><table class="table table-sm"><thead><tr><th><?= t('Type') ?></th><th><?= t('Name') ?></th><th><?= t('Expiry') ?></th><th></th></tr></thead><tbody>${docs}</tbody></table>
        <h6 class="mt-3"><?= t('My Contracts') ?></h6><table class="table table-sm"><thead><tr><th><?= t('Type') ?></th><th><?= t('Start') ?></th><th><?= t('End') ?></th><th><?= t('Status') ?></th></tr></thead><tbody>${cons}</tbody></table></div>`);
}); }
...
function loadPerf(){ $.getJSON(MH_API, { section:'performance' }, function(r){ if (!r.success) return;
    ...
    $('#pane-perf').html(`<div class="card-body"><h6><?= t('Appraisals') ?></h6><table class="table table-sm"><thead><tr><th><?= t('Cycle') ?></th><th><?= t('Overall') ?></th><th><?= t('Date') ?></th></tr></thead><tbody>${ap}</tbody></table>
        <h6 class="mt-3"><?= t('Goals') ?></h6><table class="table table-sm"><thead><tr><th><?= t('Goal') ?></th><th><?= t('Progress') ?></th><th><?= t('Status') ?></th></tr></thead><tbody>${gl}</tbody></table>
        <h6 class="mt-3"><?= t('Training') ?></h6><table class="table table-sm"><thead><tr><th><?= t('Title') ?></th><th><?= t('Type') ?></th><th><?= t('Result') ?></th><th><?= t('Cert') ?></th></tr></thead><tbody>${tr}</tbody></table></div>`);
}); }
function loadRecord(){ $.getJSON(MH_API, { section:'record' }, function(r){ if (!r.success) return;
    ...
    $('#pane-record').html(`<div class="card-body"><h6><?= t('Service Record') ?></h6><table class="table table-sm"><thead><tr><th><?= t('Date') ?></th><th><?= t('Type') ?></th><th><?= t('Title') ?></th><th><?= t('Acknowledgment') ?></th></tr></thead><tbody>${sr}</tbody></table>
        <h6 class="mt-3"><?= t('Trips') ?></h6><table class="table table-sm"><thead><tr><th><?= t('Destination') ?></th><th><?= t('Dates') ?></th><th><?= t('Status') ?></th></tr></thead><tbody>${tp}</tbody></table>
        <h6 class="mt-3"><?= t('Upcoming meetings') ?></h6><table class="table table-sm"><thead><tr><th><?= t('Title') ?></th><th><?= t('When') ?></th></tr></thead><tbody>${mt}</tbody></table></div>`);
}); }
