...
if (!empty($_SESSION['scope']['is_admin'])) {
    ...
    if (empty($sp_ids)) {
        // No project assignments — still show company-wide/untagged suppliers
        // (those with no primary project and no supplier_projects link), just
        // not anything tied to a specific project the user isn't assigned to.
        $supplier_scope_sql = " AND (s.project_id IS NULL AND NOT EXISTS (SELECT 1 FROM supplier_projects x WHERE x.supplier_id = s.supplier_id)) ";
    } else {
        $ids = implode(',', $sp_ids);
...
// Self-growing dropdown catalogues (supplier_type / payment_terms / currency).
// Options come from form_lookups; users can type a new one and it persists on save.
require_once __DIR__ . '/../../../core/form_lookups.php';
$lk_supplier_type = formLookupOptions($pdo, 'supplier_type');
$lk_payment_terms = formLookupOptions($pdo, 'payment_terms');
$lk_currency      = formLookupOptions($pdo, 'currency');
// Category options (FK) and a wide, searchable year range for the "Other"-style pickers.
...
$(document).ready(function() {
    ...
    // "Other" dropdowns: a normal searchable Select2; choosing "Other" swaps the
    // dropdown for a text input. The typed value is saved server-side and appears
    // next time. Generic — works for every .other-trigger select in any modal.
    ...
    // Choosing "Other" → hide the dropdown, reveal the text input (focused).
    $(document).off('change.otherSel').on('change.otherSel', 'select.other-trigger', function() {
        const $sel = $(this);
        const $wrap = $sel.closest('.other-field-wrap');
        const $box = $wrap.find('.other-input-box');
        if ($sel.val() === 'other') {
    ...
    // "← back to list" → restore the dropdown.
    $(document).off('click.otherBack').on('click.otherBack', '.other-back', function() {
        const $wrap = $(this).closest('.other-field-wrap');
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                const stype = response.data.supplier_type || '';
                if (stype && $('#edit_supplier_type option[value="' + stype.replace(/"/g, '\\"') + '"]').length === 0) {
                    $('#edit_supplier_type').append(new Option(stype, stype, true, true));
                }
                $('#edit_supplier_type').val(stype).trigger('change');
                $('#edit_supplier_year').val(response.data.year || '').trigger('change');
                $('#edit_category_id').val(response.data.category_id || '').trigger('change');
