...
// Dedicated standalone print page for a received invoice (Bill) — built on
// the same canonical template as print_lpo.php / invoice_print.php so this
// document type follows the same print rules (header/footer per page,
// signature row, DRAFT watermark) as every other document in BMS. Content
// mirrors received_invoices_view.php's fields exactly — this page only
// changes HOW it's printed, not what's shown.
error_reporting(0);
...
function pri_format_terms(string $t): string {
...
// Items subtotal/VAT (same math as received_invoices_view.php).
$items_sub = 0; $items_vat = 0;
foreach ($inv_items as $it) {
    $items_sub += (float)$it['quantity'] * (float)$it['unit_price'];
    $items_vat += (float)$it['tax_amount'];
}

// ── Three-approval workflow data for signature row + DRAFT watermark ──
$wf_status = $inv['status'] ?? 'pending';
...
$doc_title = ($inv['invoice_type'] === 'sub_contractor') ? 'SUB-CONTRACTOR INVOICE' : 'SUPPLIER INVOICE';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .doc-title { text-align: right; background: #3498db; print-color-adjust: exact; -webkit-print-color-adjust: exact; padding: 16px 22px; border-radius: 8px; min-width: 195px; }
        .doc-title h2 { margin: 0 0 10px 0; color: #fff; font-size: 16px; font-weight: 700; letter-spacing: 1px; print-color-adjust: exact; -webkit-print-color-adjust: exact; }
        .doc-title p { margin: 4px 0; font-size: 12px; color: #fff; print-color-adjust: exact; -webkit-print-color-adjust: exact; }
        .doc-title strong { font-weight: 600; }
        .details-grid { display: flex; flex-wrap: wrap; justify-content: space-between; margin-bottom: 24px; gap: 14px; }
        ...
        .payments-section { clear: both; padding-top: 6px; }
        .payments-section h3 { font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px; color: #1a252f; margin-bottom: 8px; }
...
<body onload="bmsAutoFitPrint()">
    ...
    <div class="header">
        ...
        <div class="doc-title">
            <h2><?= htmlspecialchars($doc_title) ?></h2>
    ...
    <div class="details-grid">
        <div class="box">
            <h3>From</h3>
            <p><strong><?= htmlspecialchars($inv['party_name'] ?? 'N/A') ?></strong></p>
            <p>Date Recorded: <?= htmlspecialchars($inv['date_recorded']) ?></p>
            <?php if (!empty($inv['payment_terms'])): ?><p>Payment Terms: <?= htmlspecialchars(pri_format_terms($inv['payment_terms'])) ?></p><?php endif; ?>
            <?php if (!empty($inv['due_date'])): ?>
            <p>Due Date: <?= htmlspecialchars($inv['due_date']) ?><?php if ($is_overdue): ?> <strong style="color:#dc3545;">(OVERDUE)</strong><?php endif; ?></p>
            <?php endif; ?>
        </div>
        <div class="box">
            <h3>Invoice Information</h3>
            <?php if ($inv['invoice_type'] === 'supplier' && !empty($inv['po_number'])): ?><p><strong>PO Reference:</strong> <?= htmlspecialchars($inv['po_number']) ?></p><?php endif; ?>
            ...
            <p><strong>Invoice Basis:</strong> <?= htmlspecialchars($inv['sc_invoice_basis'] ?? '—') ?></p>
            <p><strong>Basis Reference:</strong> <?= htmlspecialchars($inv['sc_basis_ref'] ?? '—') ?></p>
            <?php endif; ?>
            <p><strong>Recorded By:</strong> <?= htmlspecialchars($inv['recorded_by_name'] ?? 'N/A') ?></p>
            <p><strong>Created At:</strong> <?= htmlspecialchars($inv['created_at']) ?></p>
        </div>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th class="text-right" style="width:115px;">Total (<?= htmlspecialchars($currency) ?>)</th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($inv_items as $i => $it): $lt = (float)$it['quantity'] * (float)$it['unit_price']; ?>
            <tr>
                ...
                <td class="text-right"><?= number_format((float)$it['tax_rate'], 0) ?>%</td>
                <td class="text-right fw-bold"><?= number_format($lt, 2) ?></td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    ...
    <div class="totals">
        <div class="totals-row"><span>Subtotal:</span><span><?= htmlspecialchars($currency) ?> <?= number_format($items_sub, 2) ?></span></div>
        <div class="totals-row"><span>VAT:</span><span><?= htmlspecialchars($currency) ?> <?= number_format($items_vat, 2) ?></span></div>
        <div class="totals-row grand-total"><span>GRAND TOTAL:</span><span><?= htmlspecialchars($currency) ?> <?= number_format($inv_total, 2) ?></span></div>
    </div>
    <?php else: ?>
    <div class="totals">
        <div class="totals-row grand-total"><span>GRAND TOTAL:</span><span><?= htmlspecialchars($currency) ?> <?= number_format($inv_total, 2) ?></span></div>
    ...
    <?php if (!empty($inv_payments) || in_array($inv['status'], ['partial', 'paid'], true)): ?>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-right fw-bold"><?= htmlspecialchars($currency) ?> <?= number_format((float)$pmt['amount'], 2) ?></td>
                    <td><?= htmlspecialchars($pmt['payment_method'] ?? '—') ?></td>
                    <td><?= htmlspecialchars($pmt['account_name'] ?? '—') ?></td>
                    <td><?= htmlspecialchars($pmt['reference'] ?? '—') ?></td>
                    <td><?= htmlspecialchars($pmt['recorded_by_name'] ?? '—') ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
        <?php elseif ($inv['status'] === 'paid' && !empty($inv['payment_date'])): ?>
        <p>Payment Date: <?= htmlspecialchars($inv['payment_date']) ?> &nbsp;|&nbsp; Method: <?= htmlspecialchars($inv['payment_method'] ?? '—') ?><?php if (!empty($inv['payment_ref'])): ?> &nbsp;|&nbsp; Ref: <?= htmlspecialchars($inv['payment_ref']) ?><?php endif; ?></p>
        <?php endif; ?>
        <div class="totals" style="width:260px;">
            <div class="totals-row"><span>Total Paid:</span><span><?= htmlspecialchars($currency) ?> <?= number_format($amount_paid, 2) ?></span></div>
            <?php if ($amount_due > 0): ?>
            <div class="totals-row grand-total"><span>Remaining:</span><span><?= htmlspecialchars($currency) ?> <?= number_format($amount_due, 2) ?></span></div>
            <?php endif; ?>
        </div>
    </div>
    <?php endif; ?>
    ...
    <div class="notes-section">
        ...
        <div><strong>Notes:</strong><p><?= nl2br(htmlspecialchars($inv['notes'])) ?></p></div>
        <?php endif; ?>
    </div>
