<?php
// File: app/bms/invoice/invoice_print_onyx.php
// "Onyx" template — bold dark minimalist layout with a subtle wave graphic
// accent, inspired by a black-and-gray minimal freelancer invoice design.
// Same data source and fields as invoice_print.php; presentation only differs.
error_reporting(0);
...
$accent = getSetting('print_template_color_inv_onyx', '#1c1c1c');
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .company-block img { max-height: 46px; width: auto; object-fit: contain; }
        .company-block h1 { font-size: 15px; font-weight: 700; }
        .company-block .sub { font-size: 9.5px; color: #6b6b6b; margin-top: 3px; }
        .doc-title-block { text-align: right; }
        .doc-title-block h2 { font-size: 30px; font-weight: 800; letter-spacing: 1px; color: var(--accent); }
        .doc-title-block p { font-size: 11px; color: #4a4a4a; margin-top: 3px; }

        .panel-row { display: flex; justify-content: space-between; gap: 14px; margin-bottom: 18px; }
        .panel { flex: 1; }
        .panel h3 { font-size: 10.5px; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 700; margin-bottom: 6px; color: var(--accent); }
        ...
        tbody tr td { padding: 7px 6px; font-size: 11.5px; border-bottom: 1px solid #ececec; }
        .text-right { text-align: right; }
        .text-center { text-align: center; }
        .fw-bold { font-weight: 700; }

        .bottom-row { display: flex; justify-content: space-between; gap: 16px; margin-bottom: 18px; position: relative; z-index: 1; }
        .bank-details { flex: 1; }
        .bank-details h3 { font-size: 10.5px; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 700; margin-bottom: 6px; color: var(--accent); }
        ...
        .totals { width: 300px; }
        .totals-row { display: flex; justify-content: space-between; padding: 5px 0; font-size: 11.5px; border-bottom: 1px solid #ececec; }
        .totals-row.grand-total { border-top: 2px solid var(--accent); border-bottom: none; margin-top: 6px; padding-top: 8px; font-size: 14px; font-weight: 800; }
        .totals-row.balance-due { border-top: 2px solid #c0392b; border-bottom: none; margin-top: 6px; padding-top: 8px; font-size: 13px; font-weight: 700; color: #c0392b; }

        .terms-block { margin: 16px 0; font-size: 11px; position: relative; z-index: 1; }
        .terms-block strong { display: block; margin-bottom: 4px; font-size: 11.5px; color: var(--accent); }
...
<body onload="window.print()">
    ...
    <div class="header-row">
        ...
        <div class="doc-title-block">
            <h2>INVOICE</h2>
            <p>No: <?= htmlspecialchars($invoice['invoice_number']) ?></p>
    ...
    <div class="bottom-row">
        ...
        <div class="totals">
            ...
            if ($tax_amount > 0): ?>
            <div class="totals-row"><span>VAT (18%):</span><span><?= $currency ?> <?= number_format($tax_amount, 2) ?></span></div>
            <?php endif; ?>
            <div class="totals-row grand-total"><span>TOTAL</span><span><?= $currency ?> <?= number_format($invoice['grand_total'], 2) ?></span></div>
            <?php
            $paid_amount = floatval($invoice['paid_amount'] ?? 0);
            $balance_due = floatval($invoice['balance_due'] ?? $invoice['grand_total']);
            if ($paid_amount > 0): ?>
            <div class="totals-row"><span>Paid:</span><span>-<?= $currency ?> <?= number_format($paid_amount, 2) ?></span></div>
