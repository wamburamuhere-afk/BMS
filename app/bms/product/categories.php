...
<div class="product-categories-dashboard p-4 p-md-5" style="background: #ffffff; min-height: 100vh;">
    ...
    <div class="d-none d-print-block">
        <div style="text-align:center; padding: 20px 0; border-bottom: 3px solid #0d6efd; margin-bottom: 20px;">
            ...
            <h2 style="color: #000; font-weight: 600; text-transform: uppercase; margin: 5px 0; font-size: 16pt; letter-spacing: 2px;"><?= t('Product Categories Report') ?></h2>
            <p style="color: #000; margin: 0; font-size: 10pt;"><?= t('Report Date:') ?> <?= date('d M Y, H:i') ?></p>
        </div>
    </div>
    ...
    <nav aria-label="breadcrumb" class="mb-3 d-print-none">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active"><?= t('Product Categories') ?></li>
        </ol>
    </nav>
            ...
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h2 class="fw-bold mb-1">
                        <i class="bi bi-tags text-primary me-2"></i><?= t('Product Categories') ?>
                    </h2>
                    <p class="text-muted mb-0"><?= t('Manage product categories and hierarchy') ?></p>
                </div>
                <div class="d-flex gap-2">
                    <a href="<?= getUrl('products') ?>" class="btn btn-outline-secondary btn-sm px-3 shadow-sm d-flex align-items-center">
                        <i class="bi bi-arrow-left me-1"></i> <?= t('Back to Inventory') ?>
                    </a>
                    <button type="button" class="btn btn-primary btn-sm shadow-sm px-4 fw-bold" onclick="openAddModal()">
                        <i class="bi bi-plus-circle me-1"></i> <?= t('New Category') ?>
                    </button>
                </div>
            </div>
        </div>
    ...
    <div class="row">
        ...
        <div class="col-md-9">
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-header bg-white border-bottom py-3">
                    <h5 class="mb-0 fw-bold"><i class="bi bi-list-ul me-2"></i><?= t('Category List') ?></h5>
                </div>
                <div class="card-body">
                                                    ...
                                                    <li>
                                                        <a class="dropdown-item btn-edit-cat" href="javascript:void(0)"
                                                           data-cat='<?= htmlspecialchars(json_encode($cat), ENT_QUOTES, 'UTF-8') ?>'>
                                                            <i class="bi bi-pencil text-info me-2"></i> <?= t('Edit Category') ?>
                                                        </a>
                                                    </li>
                                                    ...
                                                    <li>
                                                        <a class="dropdown-item text-danger btn-delete-cat" href="javascript:void(0)"
                                                           data-id="<?= $cat['category_id'] ?>">
                                                            <i class="bi bi-trash me-2"></i> <?= t('Delete Category') ?>
                                                        </a>
                                                    </li>
                                                </ul>
                                            </div>
        ...
        <div class="col-md-3 d-print-none">
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-header bg-white border-bottom py-3">
                    <h5 class="mb-0 fw-bold"><i class="bi bi-lightbulb text-warning me-2"></i><?= t('Quick Tips') ?></h5>
                </div>
                <div class="card-body">
                    <div class="d-flex mb-3">
                        <div class="me-2"><i class="bi bi-check2-circle text-success"></i></div>
                        <div class="small text-muted"><?= t('Organize products into logical groups for easier tracking and reporting.') ?></div>
                    </div>
                    <div class="d-flex mb-3">
                        <div class="me-2"><i class="bi bi-diagram-3 text-primary"></i></div>
                        <div class="small text-muted"><?= t('Use sub-categories (Parents) to create hierarchies.') ?></div>
                    </div>
                    <div class="d-flex mb-0">
                        <div class="me-2"><i class="bi bi-eye-slash text-danger"></i></div>
                        <div class="small text-muted"><?= t('Inactive categories will not appear in product creation dropdowns.') ?></div>
                    </div>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm bg-primary text-white">
                <div class="card-body p-4 text-center">
                    ...
                    <h6 class="fw-bold mb-2"><?= t('Category Management') ?></h6>
                    <p class="small mb-0 opacity-75"><?= t('Maintain a clean hierarchy to improve inventory scanning and sales analytics.') ?></p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content border-0 shadow">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title" id="modalTitle"><?= t('New Category') ?></h5>
            ...
            <form id="categoryForm">
                ...
                <div class="modal-footer bg-light">
                    ...
                    <button type="submit" class="btn btn-primary"><?= t('Save Category') ?></button>
                </div>
            </form>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    if (!$.fn.DataTable.isDataTable('#categoriesTable')) {
        $('#categoriesTable').DataTable({
            ...
            pageLength: 25,
            ordering: false,
            language: { emptyTable: <?= json_encode(t('No categories found.')) ?>, zeroRecords: <?= json_encode(t('No matching categories.')) ?> }
        });
    }
    ...
    $('#categoryModal').on('shown.bs.modal', function () {
        if (!$('#modal_parent_id').hasClass('select2-hidden-accessible')) {
            $('#modal_parent_id').select2({
                ...
                placeholder: <?= json_encode(t('Select parent category...')) ?>,
                width: '100%'
            });
        }
...
function openAddModal() {
    ...
    $('#category_id').val('');
    $('#modalTitle').text(<?= json_encode(t('New Category')) ?>);
...
$(document).on('click', '.btn-edit-cat', function() {
    ...
    $('#modalTitle').text(<?= json_encode(t('Edit Category')) ?>);
    ...
    $.ajax({
        ...
        success: function(res) {
            if (res.success) {
                ...
                Swal.fire(<?= json_encode(t('Success')) ?>, res.message || <?= json_encode(t('Category saved')) ?>, 'success').then(() => location.reload());
            } else {
                Swal.fire(<?= json_encode(t('Error')) ?>, res.message || <?= json_encode(t('Failed to save')) ?>, 'error');
            }
        },
        error: function() {
            Swal.fire(<?= json_encode(t('Error')) ?>, <?= json_encode(t('An error occurred during communication with the server.')) ?>, 'error');
        }
    });
});
...
$(document).on('click', '.btn-delete-cat', function() {
    ...
    Swal.fire({
        ...
        text: <?= json_encode(t('This will permanently delete the category!')) ?>,
        icon: 'warning',
            ...
            $.ajax({
                ...
                success: function(res) {
                    if (res.success) {
                        ...
                        Swal.fire(<?= json_encode(t('Deleted!')) ?>, <?= json_encode(t('Category has been deleted.')) ?>, 'success').then(() => location.reload());
                    } else {
