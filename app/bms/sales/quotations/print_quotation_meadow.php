<?php
// File: app/bms/sales/quotations/print_quotation_meadow.php
// "Meadow" template — soft green rounded-panel layout, inspired by a
// Buhay Development-style green quotation design. Same data source and
// fields as print_quotation.php; presentation only differs.
error_reporting(0);
...
try {
    ...
    logActivity($pdo, $_SESSION['user_id'], 'Print Quotation',
        ($_SESSION['username'] ?? 'User') . " printed Quotation #{$order['order_number']} (Meadow template)");
...
$accent = getSetting('print_template_color_qt_meadow', '#2f7d4f');
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .meta-chip.valid-until { background: var(--accent); print-color-adjust: exact; -webkit-print-color-adjust: exact; }
        .meta-chip.valid-until .lbl, .meta-chip.valid-until .val { color: #fff; }
        ...
        .bank-details { flex: 1; background: #f5faf6; border-radius: 12px; padding: 14px 16px; }
        ...
        .totals { width: 300px; background: #f5faf6; border-radius: 12px; padding: 14px 18px; }
...
<body onload="window.print()">
    ...
    <div class="header">
        ...
        <div class="doc-title-pill"><h2>QUOTATION</h2></div>
    </div>

    <div class="meta-strip">
        <div class="meta-chip"><div class="lbl">Quote #</div><div class="val"><?= htmlspecialchars($order['order_number']) ?></div></div>
        <div class="meta-chip"><div class="lbl">Quote Date</div><div class="val"><?= date('d M Y', strtotime($order['order_date'])) ?></div></div>
        <?php if (!empty($order['quote_valid_until'])): ?>
        <div class="meta-chip valid-until"><div class="lbl">Valid Until</div><div class="val"><?= date('d M Y', strtotime($order['quote_valid_until'])) ?></div></div>
        <?php endif; ?>
        <div class="meta-chip"><div class="lbl">Status</div><div class="val"><?= strtoupper($order['status']) ?></div></div>
    </div>
