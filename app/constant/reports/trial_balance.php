...
<div class="container py-4">
    ...
    <div class="report-paper shadow mb-5" id="reportContent">
        ...
        <!-- Footer Note — screen only; the global print footer (includeFooter(),
             .bms-print-footer) already shows generator/timestamp on every printed
             page, so this would duplicate it. -->
        <div class="footer-note mt-5 px-4 pt-4 border-top d-print-none">
            <div class="row">
                <div class="col-6">
...
@media print {
    ...
    /* GRAND TOTAL row: (1) <tfoot> defaults to display:table-footer-group, which
       browsers deliberately REPEAT at the bottom of every printed page when a
       table spans multiple pages — so the total looked like it was showing up
       mid-report instead of only once, at the true end of all the data.
       table-row-group makes it render once, in normal flow, after the last body
       row. (2) it also used .h5 (~20px), nearly double the rest of the print
       table's font, so the totals didn't fit their column and wrapped onto a
       second line — same pattern already fixed on the Closing Balance row in
       Cash Flow. Shrink the font and lock column widths/no-wrap instead. */
    .account-table tfoot {
        display: table-row-group !important;
    }
    .account-table tfoot .h5 {
        font-size: 11pt !important;
    }
    .account-table {
        table-layout: fixed !important;
    }
    .account-table td.text-end {
