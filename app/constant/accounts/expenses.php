                ...
                <div class="expense-mobile-card mb-2">
                    <div class="d-flex justify-content-between align-items-start mb-1">
                        <div>
                            ...
                            ${d.recurring_profile_id ? `<span class="badge bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25" style="font-size:0.65rem;"><i class="bi bi-arrow-repeat"></i> Recurring</span>` : ''}
    ...
    const table = $('#expensesTable').DataTable({
        ...
        columns: [
            ...
            },
            {
                ...
                width: '20%',
                render: (data, t, row) => {
                    const recBadge = row.recurring_profile_id
                        ? `<span class="badge bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25 ms-1" style="font-size:0.65rem;" title="Auto-generated by recurring profile #${row.recurring_profile_id}"><i class="bi bi-arrow-repeat"></i> Recurring</span>`
                        : '';
                    return `<div><strong>${escapeHtml(data)}</strong>${recBadge}${row.notes ? `<br><small class="text-muted text-truncate d-inline-block" style="max-width:200px">${escapeHtml(row.notes)}</small>` : ''}</div>`;
                }
            },
            {
                data: 'categories',
